...
#* Read field data
field_data <-
  readRDS(here("Data/field_data.rds")) %>% .[sce_i, ] %>% 
  pull(field_sf) %>%
  .[[1]]

#* Read field parameters
field_parameters <- readRDS(here("Data/field_parameters.rds")) %>% .[sce_i, ]

#* number of simulation cases
nsim <- field_parameters$field_pars[[1]][, max(sim)]

#* Read field data
field_with_design <- readRDS(here("Data/field_with_design.rds")) %>% .[sce_i, ]
