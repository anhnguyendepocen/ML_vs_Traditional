...
## Insights into the divergence in yield and EONR prediction 
...
It is also interesting to compare the OLS and SER results. It is well-established that SER is more statistically efficient than simple OLS in the presence of unknown spatial data autocorrrelation, which is a very common in OFPE data. This advantage of SER does not seem to have received much attention as it is just a statistical property, economist have a tendency to focus on unbiasedness and consistency issues more than efficiency. However, in the context of site-specific EONR estiamtion, statistical efficiency means money. The more accurately estimated coefficients are, the more accurately site-specific EONRs are estimated, leading to a higher profit.
...
#         data = count_table,
#         title = 'Percentage (%) of each model being the best-performing among the five models in a single simulation round',
#         notes = c('A note at the bottom of the table.')
#     )
count_table <- prt_table %>% 
    #=== Wide to Long: melt()
...
```{r pair-table-yield, echo=FALSE, cache=FALSE}
model_list <- unique(est_data$model) %>% .[order(.)] %>% as.character()
pair_df <- data.frame()
for(i in 1:length(model_list)){
    for(j in 1:length(model_list)){
        pi <- est_data %>% 
            filter(field_size=="37.3 ha", model==model_list[i]) %>% 
            pull(rmse_cv)
        pj <- est_data %>% 
            filter(field_size=="37.3 ha", model==model_list[j]) %>% 
            pull(rmse_cv)
        pv <- t.test(pi, 
                     pj, 
                     paired = TRUE)
        ...
        pair_df[as.character(i), model_list[j]] <- 
            paste0("(", round(pv$p.value, 3), ")")
    }
}
pair_df[is.na(pair_df)] <- ""
knitr::kable(pair_df, format = "markdown",
             caption = "Pairwise t-test of predicted yield RMSE between models, Large field (37.3 ha), values are the difference in RMSE (row vs. index) (kg/ha), p-values in parenthese")
```

```{r pair-table-eonr, echo=FALSE, cache=FALSE}
model_list <- unique(est_data$model) %>% .[order(.)] %>% as.character()
pair_df <- data.frame()
for(i in 1:length(model_list)){
    for(j in 1:length(model_list)){
...
pair_df[is.na(pair_df)] <- ""
knitr::kable(pair_df, format = "markdown",
             caption = "Pairwise t-test of predicted EONR RMSE between models, Large field (37.3 ha), values are the difference in RMSE (row vs. index) (kg/ha), p-values in parenthese")
```

```{r pair-table-profit, echo=FALSE, cache=FALSE}
model_list <- unique(est_data$model) %>% .[order(.)] %>% as.character()
pair_df <- data.frame()
for(i in 1:length(model_list)){
    for(j in 1:length(model_list)){
...
pair_df[is.na(pair_df)] <- ""
knitr::kable(pair_df, format = "markdown",
             caption = "Pairwise t-test of profits between models, Large field (37.3 ha), values are the difference in profits (row vs. index) (kg/ha), p-values in parenthese")
```
