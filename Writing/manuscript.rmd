...
## Yield predictions
Machine learning methods perform better than traditional regression methods in yield prediction.  The RMSE results for the out-of-sample yield prediction over the 1,000 simulations are displayed in Figure \@ref(fig:rmse-yield-boxplot).  

- For the full field (37.3 ha) scenario, the average RMSE over 1,000 simulations is `r mean_data[field_size=="37.3 ha"&model=="RF", rmse_yield] %>% round(2)` kg ha$^{-1}$ for the RF model, and `r mean_data[field_size=="37.3 ha"&model=="BRF", rmse_yield] %>% round(2)` kg ha$^{-1}$ for the BRF model, which are significantly smaller than the LM (`r mean_data[field_size=="37.3 ha"&model=="LM", rmse_yield] %>% round(2)` kg ha$^{-1}$) and SER (`r mean_data[field_size=="37.3 ha"&model=="SER", rmse_yield] %>% round(2)` kg ha$^{-1}$) models.  If we give RF model the best data, its RMSE will be as small as `r mean_data[field_size=="37.3 ha"&model=="RF best", rmse_yield] %>% round(2)` kg ha$^{-1}$.
...
EONR prediction performances for different models are displayed in Figure \@ref(fig:rmse-eonr-boxplot).


## Profit performances

Profit performance results for different models are displayed in Figure \@ref(fig:pi-boxplot).
...
```{r rmse-yield-boxplot, fig.cap="The yield prediction performances of different models.", echo=FALSE, fig.width=7}
rmse_yield_boxplot
```

```{r rmse-eonr-boxplot, fig.cap="The yield prediction performances of different models.", echo=FALSE, fig.width=7}
rmse_eonr_boxplot
```

```{r pi-boxplot, fig.cap="The profit performances of different models.", echo=FALSE, fig.width=7}
pi_boxplot
```
