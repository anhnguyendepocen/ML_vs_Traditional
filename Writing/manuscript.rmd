...
- Within the machine learning models, the RF model performed especially poorly in EONR prediction. Its EONR prediction RMSE was much larger than BRF model, and its associated profit was extremely low (as shown in Figure \@ref(fig:comb-boxplot-all) panel (A)). 
- Even if we "cheat" and feed RF model with the best possible data (i.e., the true yield response parameters), its EONR profit ($\$$`r mean_data[field_size=="37.3 ha"&model=="RF_b", profit] %>% round(2)` ha$^{-1}$) was still slightly lower than BRF model ($\$$`r mean_data[field_size=="37.3 ha"&model=="BRF", profit] %>% round(2)` ha$^{-1}$) that was estimated using observed messy data. 
- It is noteworthy to mention that RF model is currently a very popular machine learning method used in many agricultural studies. Our simulation results showed that although RF may be OK for yield prediction, it is terrible in EONR prediction, and will cause big economic loss if using it in farming recommendation development. Even after spending great efforts (and financial costs) to obtain the perfect information, using RF model still won't achieve good economic performance.
...
Why good yield prediction does not necessarily end up with good economic performance? That sound counterintuitive. We attempt to provide some explanations. Note this is not a statistical paper and providing the rigorous statistical proof is beyond the scope of this paper. We just try providing some illustrative explanations.

- Figure \@ref(fig:single-simu-illustrate) showed the predicted versus true yield (panel A) and EONR (panel B) of the 1,440 locations ($aunit$) from one example simulation's results (large field). The RMSE measurements associated with each model's results for this specific simulation were displayed in the figure as well. Though the machine learning models (RF, RF_b, and BRF) generated well-predicted yields such that the plotted points were well distributed along the 45 degree line, their predicted EONR were not continuous but fell into several discrete values. That results were to a large extent caused by the discrete data of five nitrogen trial rates. But that is how the on-farm experimental data generally look like in practice. On the other hand, the SER model's predicted EONR were much better aligned the 45 degree line, despite its predicted yields were not as good as the machine learning models. The OLS model's EONR predictions are also continuous, but with too big dispersion. 
- We further look into the yield response curve estimation at individual locations. Figure \@ref(fig:yield-curves) illustrated the estimated yield response curves by different models at a specific location ($aunit$). The circle point denoted the trial N rate and true (deterministic part of) yield. This figure showed that the machine learning models' predicted yields were closer to the true yield. However, the shapes of yield response curves predicted by machine learning models were much less accurate. On the other hand, SER model's predicted yield levels were heavily biased, but its predicted yield response curve had the shape much more resembling the true response curve. The EONR is determined by the yield response (mathematically, the slope of the curve), rather than the yield level. 
- Mathematically the slope of the curve: economics term, marginal effect, but the derivative measure is not commonly used in agronomy studies; Agronomy term, treatment effect, which is not the continuous derivative measure, but regarding the different N levels as discrete values. A treatment effect is defined as the change in yield from one N rate to the next level N rate. Here we have five N treatment rates, and therefore define four treatment effects. The estimated versus true treatment effects of N rates for the same simulation round's 1,440 locations were shown in Figure \@ref(fig:treatment-effect) for different models. Notice that we used the quadratic-plateau function as the true yield response in the simulation, and therefore at higher N levels the yields are more likely to approach or pass the plateau and the true treatment effect is quite small or even zero. As can be seen, the machine learning models mostly underestimated the treatment effects at all N levels. That is the major reason why machine learning models failed to predict accurate EONR. In contrast, SER model did a much better job in treatment effect predictions, particularly at the low N levels. OLS model's estimated treatment effects were actually less biased compared to machine learning models, but the variations were just very large.
- (Maybe we don't need to say this, as this is so obvious.) Caveat: The single simulation round we choose was a little cherry-picking. We chose that the round where the five models' profit results were roughly corresponding to the means as shown in Figure \@ref(fig:comb-boxplot-all). Indeed, there are lots of variations across simulation rounds, such that in come simulations machine learning models outperformed SER model. We believe the one example simulation we showed here is representative, and can make a good illustration of the yield versus EONR prediction performances.  


## OLS vs. SER: Statistical efficiency is money

(I feel this part is only a minor point in this study. Perhaps it's not important enough to be put in the title.)

- It is also interesting to compare the OLS and SER results. The good performance of SER may imply some good suggestion to production.
- It has been well known in the econometric literature that SER model can improve the estimation efficiency compared to OLS at the presence of unknown spatial data autocorrrelation, which is a very common in crop field characteristics data. 
- However, since it is only an efficiency issue, and SER does not correct for bias, this advantage of SER model has not received enough attention. The efficiency improvement is only regarded as a statistical property, while its economic benefit is not well recognized.
- Our simulation evidence showed that the statistical efficiency of SER model can actually translate into considerable more profitable EONR recommendations. The profit of SER model is higher than OLS model by $\$$`r (mean_data[field_size=="37.3 ha"&model=="SER", profit] - mean_data[field_size=="37.3 ha"&model=="OLS", profit]) %>% round(2)` ha$^{-1}$, $\$$`r (mean_data[field_size=="18.7 ha"&model=="SER", profit] - mean_data[field_size=="18.7 ha"&model=="OLS", profit]) %>% round(2)` ha$^{-1}$, and $\$$`r (mean_data[field_size=="9.3 ha"&model=="SER", profit] - mean_data[field_size=="9.3 ha"&model=="OLS", profit]) %>% round(2)` ha$^{-1}$ for large, medium, and small fields, respectively.
- So, using SER model to update the OLS model is likely to be economically beneficial in on-farm experimental data analysis, and should be encouraged. 
